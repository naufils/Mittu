...
import { Text, View, FlatList, StyleSheet, Image, ScrollView, TouchableOpacity, StatusBar, RefreshControl,BackHandler, ActivityIndicator } from 'react-native';
      ...
      if(this.state.renderItems.length !==0){
        ...
        return (
          ...
          <View style={{ flex: 1, flexDirection:'column', alignItems: 'flex-start', backgroundColor:'#121212', marginBottom:10, marginTop:10}} key={i}>              
            <Text style={{padding:7,color:'#F9A818', fontFamily:'OpenSans-Bold',fontSize:16}}>{item[0].view_name}</Text>
            <ScrollView horizontal> 
            <FlatList
              horizontal={true}
              data={item}
              renderItem={({item, index, separators}) => (
                <View style={{paddingLeft:5, paddingRight:1, marginTop:1, marginRight:5, flexDirection:'row', width:114}} key={i}>
                  <TouchableOpacity onPress={() => this.onPressImage(item)}>
                    <Image style={{width: 110, height: 140,backgroundColor:'grey', borderWidth:1, borderRadius:5,}} source={{uri: item.vid_thumbs}} />
                    <Text style={{fontFamily:'OpenSans-Regular',width: 110,color:'white',flex:1, flexWrap:'wrap'}}>{item.vid_name}</Text>
                  </TouchableOpacity>
                </View>
              )}
            />        
