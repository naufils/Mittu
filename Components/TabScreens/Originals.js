...
import { Text, View, FlatList, StyleSheet, Image, ScrollView, TouchableOpacity, Dimensions, StatusBar, RefreshControl, BackHandler, ActivityIndicator } from 'react-native';
...
class Featured extends React.Component {
 ...
 handleBackPress = () => {
  ...
  _renderItem = (item)  => {
    ...
    return (
      <View style={{paddingTop:0, paddingRight:5.2, paddingLeft:6.3, paddingBottom:1, width:DEVICE_WIDTH}} key={item.index}>
      <TouchableOpacity onPress={() => this.onPressImage(item.item)}>
        <Image style={{width: '100%', height: 270, borderRadius:5,backgroundColor:'grey'}} source={{uri: item.item.vid_thumbs}} />
      </TouchableOpacity>
    </View>
    );
}
    ...
    render() {
      ...
      if(this.state.renderItems.length !==0){
        ...
        let featureFilter= this.state.renderItems.map(item => item.filter(item1 => item1.feature_name !== "Carousel"))
      ...
      featuredList = featureFilter.map((item,i) => {
        ...
        return (
          ...
          <View style={{ flex: 1, flexDirection:'column', alignItems: 'flex-start', backgroundColor:'#121212', marginBottom:10, marginTop:1}} key={i}> 
            <Text style={{padding:7,color:'#F9A818',fontFamily:'OpenSans-Bold',fontSize:16}}>{featname}</Text>
            <ScrollView horizontal> 
            <FlatList
              horizontal={true}
              data={item}
              renderItem={({item, index, separators}) => (
                <View style={{paddingLeft:5, paddingRight:1, marginTop:1, flexDirection:'row', marginRight:5, width:114}} key={index}>
                  <TouchableOpacity onPress={() => this.onPressImage(item)}>
                    ...
                    <Text style={{fontFamily:'OpenSans-Regular',width: 110, color:'white',flex:1, flexWrap:'wrap', paddingTop:5}}>{item.vid_name}</Text>
                  </TouchableOpacity>
                </View>
              )}
            />        
            ...
            <View style={{flex:1, paddingTop:10}}>
              <View>
                  <Carousel
                    ...
                    firstItem={0}
                    onBeforeSnapToItem={(slideIndex) => console.log("slideIndex", slideIndex)}
                  />
